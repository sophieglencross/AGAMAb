...
namespace df {
	...
	EXP QuasiSphericalCOM::QuasiSphericalCOM(
		const math::IFunction& density, const math::IFunction& potential, double _beta0, double _r_a, double epsilonJ)
		:
		QuasiSpherical(potential, epsilonJ), invPhi0(1. / potential(0)), beta0(_beta0), r_a(_r_a),
	...
	class Isnn : public math::IFunctionNoDeriv {
	private:
		const int ns, nc;
		const double chi, alpha;
	public:
		Isnn(const int _ns, const int _nc, const double _chi, const double _alpha) :
			ns(_ns), nc(_nc), chi(_chi), alpha(_alpha) {
		}
		virtual double value(const double eta) const {
			double c, s; math::sincos(eta, s, c);
			return nc == 0 ? pow(s, ns) / pow(1 + chi * s * s, alpha) :
				pow(s, ns) * pow(c, nc) / pow(1 + chi * s * s, alpha);;
		}
		double I(void) {
			return math::integrate(*this, 0, .5 * M_PI, 1e-6);
		}
	};

	class CuddDeltaI : public math::IFunctionNoDeriv {//integrate Kernel*asymptotic
	private:
		const double Phi, raor, alpha, indx, two32;
	public:
		CuddDeltaI(const double _Phi,
			const double _raor, const double _alpha, const double _indx) :
			Phi(_Phi), raor(_raor), alpha(_alpha), indx(_indx),
			two32(pow(2, 1.5)) {
		}
		double K(const double H) const {//the kernel
			double chi = (Phi / H - 1) / pow_2(raor);
			Isnn I(1, 0, chi, alpha);
			return sqrt(H - Phi) * I.I();
		}
		virtual double value(const double H) const {
			return pow(-H, indx) * K(H);
		}
		double IK(const double H1, const double H2) {
			return two32 * M_PI * math::integrate(*this, H1, H2, 1e-6);
		}
	};

	void createCuddDF(const math::IFunction& density, const math::IFunction& potential, double alpha, double r_a, int n , std::vector<double>& Esp, /*output*/ std::vector<double>& gsp) {
		double thetamax = 8;//controls max radius
		int np = 400;
		std::vector < double>r(np), Phi(np), rho(np);
		for (int i = 0; i < np; i++) {//set up radial grid with rho and Phi
			double theta = i * thetamax / (double)(np - 1);
			//std::cout << i << " log(r): " << r[i] << " r: " << exp(r[i]) << "\n";
			r[i] = r_a* sinh(theta);   
			Phi[i] = potential(r[i]);
			rho[i] = density(r[i]);
		}
		math::CubicSpline rPhi(Phi, r);
		math::CubicSpline rhoPhi(Phi, rho);

		double indx = n - 1.5 - 2 * alpha;//asymptotic form of g
		std::vector<double> Hs(np), Es, gs;
		double dH = -Phi[0] / (double)(np - 1);// dH>0
		for (int i = 0; i < np; i++) {//grid on which to evaluate g
			Hs[i] = -i * dH - 1e-20;
		}
		for (int i = 0; i < np - 1; i++) {
			CuddDeltaI CD(Hs[i + 1], r_a / rPhi(Hs[i + 1]), alpha, indx);
			double sum = 0;
			for (int j = i - 1; j >= 0; j--) sum += gs[j] * CD.IK(Hs[j + 1], Hs[j]);
			double g = (rhoPhi.value(Hs[i + 1]) - sum) / CD.IK(Hs[i + 1], Hs[i]);
			Es.push_back((Hs[i] + Hs[i + 1]) / 2);
			gs.push_back(g);
		}
		int nm = Es.size();

		for (int i = 0; i < gs.size(); i++) gs[i] *= .5 * pow(-Es[i], indx);

		for (int i = 0; i < nm; i++) {  // Reverse the arrays for the interpolator
			Esp.push_back(Es[nm - 1 - i]); gsp.push_back(gs[nm - 1 - i]);
		}
	}


	math::LogLogSpline createCuddDF(
		const math::IFunction& density, const math::IFunction& potential, double alpha, double r_a, int n)
	{
		std::vector<double> gridPhi, gridf;
		createCuddDF(density, potential, alpha, r_a,n, /*output*/ gridPhi, gridf);
		...
		// This reversal is now done in createCuddDF
		return math::LogLogSpline(gridPhi, gridf);
	}



	//------ Cudd DF class for Cuddesford-Lewis models -----//
	
	EXP Cudd::Cudd(
		const math::IFunction& density, const math::IFunction& potential, double _alpha, double _r_a, int _n, double _epsilonJ)
		:
		QuasiSpherical(potential, _epsilonJ), invPhi0(1. / potential(0)), alpha(_alpha), r_a(_r_a), n(_n),
		df(createCuddDF(density, potential, alpha, r_a, n))
	{
	}

	EXP double Cudd::value(double E, double L, double Lz) const
	{
		
		double P = invPhi0 - 1. / E;
		double k = pow_2(L / r_a) / (2 * fabs(E));
		if (L<0 || fabs(Lz)>L || P < 0)
			return NAN;   // invalid range - this shouldn't happen in normal circumstances, so be picky here
		// note that we don't (and cannot) check that L <= Lcirc(E)
		return df(P) * (alpha == 0 ? 1 : math::pow(1 + k, -alpha));
	}
}  // namespace df
